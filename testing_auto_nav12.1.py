...
def move():
    ...
    if forward_right()!=False:
        if forward_right()[1]==0:
            rotatebot(-70)
            rospy.loginfo(['Rotating -70 , forward right()!=False'])
        elif abs(laser_range[30]-laser_range[-30])<0.2:
            rospy.loginfo('It is a trap!!!')
            rotatebot(10)
            move()
    elif forward_left()!=False:
        if forward_left()[1]==0:
            rotatebot(70)
            rospy.loginfo(['Rotating -70 , forward left()!=False'])
        elif abs(laser_range[30]-laser_range[-30])<0.2:
            rospy.loginfo('It is a trap!!!')
            rotatebot(10)
            move()
...
def pick_direction():
    ...
    if laser_range.size != 0:
        move()
        if forward_right()!=False:
            if laser_range[10]>laser_range[-10]:
                lr2i=10
            else:
                lr2i=-10
        elif forward_left()!=False:
