    ...
    for y in range(msg.info.height):
            for x in range(msg.info.width):
                    if pixels[y][x]==0:
                            ...
                            unmapped_distance.append(sqrt((x-i_centerx)**2+(i_center-y)**2))
    b=np.array([0,0])
    c=np.array([0,1])
    for i in lst:
